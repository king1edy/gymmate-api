...
export class TenantService {
  ...
  async create(createTenantDto: CreateTenantDto): Promise<Tenant> {
    ...
    const tenant = this.tenantRepository.create({
      ...
      limits: { maxMembers: 100, maxStaff: 5 }, // Starter plan limits
      featuresEnabled: [
        'basic_analytics',
        'member_management',
        'class_scheduling',
  ...
  async updateSubscription(
    ...
    switch (plan) {
      ...
      case SubscriptionPlan.ENTERPRISE:
        tenant.limits = { maxMembers: -1, maxStaff: -1 }; // Unlimited
        tenant.featuresEnabled = [
          'basic_analytics',
          'advanced_analytics',
          'member_management',
  ...
  async checkUsageLimit(
    ...
    if (
      tenant[`max${limitType.charAt(0).toUpperCase() + limitType.slice(1)}`] ===
      -1
    ) {
