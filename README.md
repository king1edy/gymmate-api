...
- Create Member (admin only): `POST /create-member` (requires Bearer token)
- List Trainers (any logged-in user): `GET /list-trainers` (requires Bearer token)
- Book Class (member only): `POST /book-class` { classId } (requires Bearer token)
...
- src/
  ...
  - auth/
    ...
    - roles.decorator.ts
    - roles.guard.ts
  - gym/ (future: gym management features)
  - class/ (future: class booking features)

## Configuration
- Uses @nestjs/config for environment management.
- All sensitive values (DB, JWT, etc.) are loaded from environment variables.

## Production Best Practices
- Global validation pipe enabled.
- Modular, feature-based folder structure.
- Use environment variables for all secrets and DB config.
- Set `synchronize: false` in TypeORM for production.
- Add HTTPS, logging, and monitoring as needed.
